...
function parseUrl(url) {
  if (/^\/\//.test(url)) {
    url = ((window && window.location) ?
      window.location.protocol : 'http:') + url;
  } else if (/^\//.test(url) && window && window.location) {
...
function request(method, url, options) {
  ...
  var req = http.request({
    ...
    if (res.statusCode !== 200) {
      resolver.reject("Request to " + url + " failed with " + res.statusCode);
      return;
    }
    ...
    res.on('end', function() {
      ...
      if (contentType.indexOf('application/json') !== -1) {
        try {
          ...
          resolver.reject("Could not parse response body as JSON: " + err);
          return;
        }
      }
      resolver.resolve([result, res]);
    });
  });
